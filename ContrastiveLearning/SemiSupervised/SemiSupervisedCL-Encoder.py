...
# The encoder extracts representation vectors from MRI scans.
def get_encoder():
...
class ContrastiveModel(keras.Model):
    def __init__(self):
        ...
        # add a nonlinear projection head to the encoder
        self.projection_head = keras.Sequential([keras.Input(shape=(width,)),layers.Dense(width, activation="relu"),layers.Dense(width)])
    ...
    def contrastive_loss(self, projections_1, projections_2):
        # have a pair of correlated views for the same scan
        projections_1 = tf.math.l2_normalize(projections_1, axis=1)
        projections_2 = tf.math.l2_normalize(projections_2, axis=1)
        # use the temperature-scaled cosine similarity 
        similarities = (projections_1@projections_2.T)/self.temperature)

        batch_size = tf.shape(projections_1)[0]
        contrastive_labels = tf.range(batch_size)
        self.contrastive_accuracy.update_state(contrastive_labels, similarities)
        self.contrastive_accuracy.update_state(contrastive_labels, similarities.T)

        loss_1_2 = keras.losses.sparse_categorical_crossentropy(contrastive_labels, similarities, from_logits=True)
        loss_2_1 = keras.losses.sparse_categorical_crossentropy(contrastive_labels, similarities.T, from_logits=True)
